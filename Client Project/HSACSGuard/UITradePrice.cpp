...
{
	m_btnKC = g_pMain->GetChildByID(m_dVTableAddr, "btn_iskc");
	m_edit_money = g_pMain->GetChildByID(m_dVTableAddr, "edit_money");
	m_txtMoney = g_pMain->GetChildByID(m_edit_money, "text_money");
}
...
void CUITradePricePlug::Tick()
{
	if (g_pMain->IsVisible(m_dVTableAddr))
	{
		if (updateRequired) {
			...
			g_pMain->SetState(m_btnKC, m_bIsKC ? UI_STATE_BUTTON_DOWN : UI_STATE_BUTTON_NORMAL);
		}
...
{
	g_pMain->pClientTradePrice->ReceiveMessage(pSender, dwMsg);
