...
void CPlayerBase::UpdateKC(Packet& pkt)
{
	...
	if (g_pMain->pClientUIState != NULL)
		g_pMain->pClientUIState->UpdateKC(m_iKC,iBalance);

	if (g_pMain->pClientTradeInventory != NULL)
		g_pMain->pClientTradeInventory->UpdateTotal(m_iKC);
...
void CPlayerBase::UpdateStats(Packet& pkt)
{
	...
	//g_pMain->m_PlayerBase->UpdateFreeStatPoints(m_iFreeStatPoints);
}
...
void CPlayerBase::ResetStats(Packet& pkt)
{
	...
	//g_pMain->m_PlayerBase->UpdateFreeStatPoints(free);
}

void CPlayerBase::UpdateFreeStatPoints(uint16 freePoints)
{
	m_iFreeStatPoints = freePoints;
	//g_pMain->SetString(m_HookMain->pClientHookManager->pClientUIState->m_strFreeStatPoint, std::to_string(m_iFreeStatPoints));
}
