...
{
	uint32 pIndex;
	uint16 sPerkCount, sMaxPerkCount;
	bool sStatus, sPercentage;
	std::string strDescp;
};

struct _PERKS_DATA_INFO
{
	uint16 sPerkType[PERK_COUNT], sRemPerk;
	uint32 sPerkCoins;

	_PERKS_DATA_INFO() { Initialize(); }

	void Initialize() {
		memset(sPerkType, 0, sizeof(sPerkType));
		sRemPerk = 0;
		sPerkCoins = 0;
	}
};

enum class PerkErrorCode
{
	remPerks,
	notFound,
	maxPerkCount,
...
#define ZONE_ZINDAN_WAR			91
	...
	uint8 arrowAc;
	uint8 spearAc;
	bool isTeleporting, isRankOpen, RankPacketReceived, m_iClanBank;
	bool logOut;
	uint8 ZoneID;
	int64 HWID;
	vector<string> GPU;
	...
	INLINE uint8 GetZoneID() { return ZoneID; }
...
struct HSACSX_NEW_EVENT_SYSTEM
{
	bool m_bStatus;
	uint16 m_sEventTimer;

	HSACSX_NEW_EVENT_SYSTEM()
	{
		Clean();
	}
...
#define HOOK_MODE 0					// GShare Hooking
...
struct EventShowList {
	std::string name, time;
	uint32 hour, minute;
	EventShowList(std::string name, std::string time, uint32 hour, uint32 minute) {
		this->name = name;
		this->time = time;
		this->hour = hour;
		this->minute = minute;
