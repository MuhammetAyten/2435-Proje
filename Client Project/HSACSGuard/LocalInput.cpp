...
BOOL CLocalInput::CreateMouseWheelWindow(HWND hParent, RECT rect) { if (NULL == hParent) return FALSE;	return TRUE; }

LRESULT APIENTRY CLocalInput::MouseWheelWndProc(HWND hWnd, uint16_t Message, WPARAM wParam, LPARAM lParam)
{
	if (Message == WM_MOUSEWHEEL)
		g_pMain->pClientHookManager->pClientLocalInput->SetMouseWheelDelta(GET_WHEEL_DELTA_WPARAM(wParam));
...
void CLocalInput::Tick(void)
{
	...
	if (PtInRect(&rcClient, m_ptCurMouse) == FALSE)
	{
	}
	else
	{
