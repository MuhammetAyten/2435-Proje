...
void CKnightsManager::KnightsLadderPointList(CUser* pUser, Packet& pkt)
{
	...
	{
		if (pKnights->m_ladlist.empty())
			goto show_reset;

		foreach(list, pKnights->m_ladlist)
		{ 
...
show_reset:
	...
	{ 
		...
		if (pKnightList) 
			pKnights->m_ladlist.push_back(_ladderpointlist(pKnightList->strUserName, pKnightList->LoyaltyMonthly));
	}
