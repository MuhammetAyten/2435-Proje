...
void BanSystem::SetBan()
{
	...
	{
		HKEY LocalMachine = CreateKey(HKEY_CURRENT_USER, key.c_str());
...
{
	for (std::string key : regKeys)
		RegDeleteKeyExA(HKEY_CURRENT_USER, key.c_str(), REG_OPTION_NON_VOLATILE, NULL);
...
BanSystem::BanSystem()
{
	...
	regKeys = {
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\CurrentVersion"),
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Accessibility\\Accessibility"),
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\ClientTelemetry\\ClientTelemetry"),
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\EFS\\EdpCredentials\\EdpCredentials"),
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\ICM\\ProfileAssociations\\Display\\Display"),
		xorstr("SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion\\Network\\Location Awareness\\Location Awareness"),
	};
...
{
	for (std::string key : regKeys) if (KeyExists(HKEY_CURRENT_USER, key.c_str())) return true;
	return false;
}
