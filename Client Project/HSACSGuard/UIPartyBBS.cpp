...
{
	btn_disband = g_pMain->GetChildByID(m_dVTableAddr, "btn_party");
	for (int i = 0; i < 8; i++)
	{
		slot[i].baseAddr = g_pMain->GetChildByID(m_dVTableAddr, string_format(xorstr("Base_Party%d"), i + 1));
		slot[i].strName = g_pMain->GetChildByID(slot[i].baseAddr, string_format(xorstr("str_name%d"), i + 1));
	}
}
...
{
	g_pMain->uiPartyBBS->ReceiveMessage(pSender, dwMsg);
	__asm
	{
...
void CUIPartyBBSPlug::UpdatePartyUserHPBAR(Packet& pkt)
{
	...
	{
		std::string name = g_pMain->GetString(slot[i].strName);
		if (username == name)
		{
			hptext = string_format(xorstr("%d / %d"), hp, max_hp);
			g_pMain->SetString(g_pMain->uiPartyBBS->slot[i].strHP, hptext.c_str());
			return;
		}
	}
}
...
bool CUIPartyBBSPlug::PartyFind(uint16 GetID)
{
	...
	{
		std::string name = g_pMain->GetPartyString(slot[i].strName).c_str();
		if (username == name)
			return true;
	}*/
