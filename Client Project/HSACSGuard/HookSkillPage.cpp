...
{
	m_orgbtnResetSkill = g_pMain->GetChildByID(m_dVTableAddr, "btn_reset");
	m_btnResetSkill = g_pMain->GetChildByID(m_dVTableAddr, "btn_reset_skill");
	m_btn4 = g_pMain->GetChildByID(m_dVTableAddr, "btn_4");
	m_btn5 = g_pMain->GetChildByID(m_dVTableAddr, "btn_5");
	m_btn6 = g_pMain->GetChildByID(m_dVTableAddr, "btn_6");
	m_btn7 = g_pMain->GetChildByID(m_dVTableAddr, "btn_7");
}
...
bool HookSkillPage::ReceiveMessage(DWORD* pSender, uint32_t dwMsg)
{
	...
	if (!pSender || pSender == 0 || dwMsg != UIMSG_BUTTON_CLICK)
	{
		if (pSender == (DWORD*)m_btnResetSkill) {
			std::string reqcoins = "Would you like to reset your skill points for :    " + std::to_string(g_pMain->Player.moneyreq) + " coins.";
			g_pMain->pClientHookManager->ShowMessageBox("Skill Reset", reqcoins, MsgBoxTypes::YesNo, ParentTypes::PARENT_SKILL_TREE);
		}
...
{
	g_pMain->uiSkillPage->ReceiveMessage(pSender, dwMsg);
